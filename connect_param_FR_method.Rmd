...
#land class
lc <- raster(here::here("data/land_use_SE27.tif"))# whole of eng and wales
crs(lc) <- bng

#roads
roads <- raster(here::here("data/roads_SE27.tif"))
crs(roads) <- bng
...
slope <- raster(here::here("data/slope_25m_EW.tif"))
crs(slope) <- bng
slope_se27 <- resample(slope, roads)
writeRaster(slope_se27, "data/slope_25m_SE27_resampled.tif",overwrite=TRUE)
...
lf <- raster("data/lf_SE27_nfi_masked.tif")
crs(lf) <- bng
...
param.table <- expand.grid(pd,md,wts,tr, road_layers, lf_layers, tri_layers)
names(param.table) <- c("pd","md","wts","land_cover","road_layers","lf_layers", "tri_layers")
...
params.kernel <- subset(param.table[1:2])
#remove duplicates
...
# loop through list #
for(j in 1:nrow(params.kernel[1:5,])) { #Parameter table row = j, using rows 1-5 for example
...
connect_sum <- all.dat %>%
  select(focal_id, coninter, conintra) %>%
  group_by(focal_id) %>%
...
ggplot(data = connect_sum$geometry) +
  geom_sf(aes(fill = (connect_sum$total_coninter_log)), size = 0.2) +
  scale_fill_viridis_c(option = "inferno", name = "Log Interconnectivity")

#Plot conintra

ggplot(data = connect_sum$geometry) +
  geom_sf(aes(fill = (connect_sum$total_conintra_log)), size = 0.2) +
  scale_fill_viridis_c(option = "inferno", name = "Log Intraconnectivity")
...
for(j in 1:nrow(param.table[1:5,])) { #Parameter table row = j, using rows 1-5 for example
      ...
      for (k in 1:length(all.tr.layers[1:5])) { #Transition layer = k
  ...
  #calculate dispersal contribution
  dispersal_contribution <-
  -((log(1 / param.table$pd[j])) / param.table$md[j]) # a is % disprsers, b is buffer size
  
  #buffer cutoff
  buffer_cutoff <- round(log(1 / (1 - dispersal_cutoff)) / (log(1 / param.table$pd[j]) /
                                             param.table$md[j]), digits = 2)
  ...
  #calculate least-cost distance between centroids
  lc_dist <- as.vector(gdistance::costDistance(all.tr.layers[[k]], focal_centroid[1], source_centroid[1]))
  #If lc_dist = Inf, convert to 0
  # create a data frame with all information in
  Conn_table_site <- data.frame(focal_patch = focal_patch$cluster,
                                focal_patch_area = focal_patch$clstr_r,
                                source_patch = source_patch$cluster, 
                                source_patch_area = source_patch$clstr_r,
                                least_cost_dist = lc_dist,
                                buffer = param.table$md[j],
                                perc.disp = param.table$pd[j],
                                combo_id = param.table$combo_id[j]) %>%
    mutate(lc_distfin = least_cost_dist/cellres) %>%
    mutate(lc_distfin = as.numeric(lc_distfin)) %>%
    ### Calculate interconnectivity value
    mutate(coninter = (exp(-((log(1/param.table$pd[j]))/param.table$md[j])*lc_distfin)*(focal_patch_area^2))) %>%
    ### Calculate incoming connectivity
    mutate(incoming_connect = case_when(focal_patch == source_patch ~ NA_real_,
...
ggplot(data = connect_sum_lc$geometry) +
  geom_sf(aes(fill = (connect_sum_lc$total_coninter_log)), size = 0.2) +
  scale_fill_viridis_c(option = "inferno", name = "Log Interconnectivity")

#Plot conintra

ggplot(data = connect_sum_lc$geometry) +
  geom_sf(aes(fill = (connect_sum_lc$total_conintra_log)), size = 0.2) +
  scale_fill_viridis_c(option = "inferno", name = "Log Intraconnectivity")
